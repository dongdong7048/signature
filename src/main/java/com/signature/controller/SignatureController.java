...
public class SignatureController {
    ...
    public String index(@RequestParam("pdfname") String pdfname, @RequestParam("usertype") String usertype, HttpServletRequest request, Model model) throws java.io.IOException {
        ...
        //pdfname = transHttpPathToJavaPath(pdfname);
        log.info("===" + actionName.SHOW_PDFPAGE + "===");
        //String TargetPDFpathWithoutExtension = Desktop_temp_DirectoryPath + getFilenameWithoutExtension(pdfname);
        recordAction(0, pdfname, actionName.SHOW_PDFPAGE, getNowTime(), null, usertype, null, null);
        ...
        //request.setAttribute(Current_pdfnameWithoutExtension,Current_pdfnameWithoutExtension);
        //File file  = new File(Desktop_temp_DirectoryPath+"image"+index+".png");
        //File file = new File(Desktop_temp_DirectoryPath + "image11.png");
...
//        response.getOutputStream().write(bytes);
    ...
    public Signature checkFileIsExistAndReturnSignature(String pdfname) {
        ...
        if (signature == null) {
            signature = signatureMapper.findDataByPdfFilename(pdfname_suffix_H);
            if (signature == null) {
                signature = signatureMapper.findDataByPdfFilename(pdfname_suffix_A);
                if (signature == null) {
                    return null;
                } else {
                    return signature;
    ...
    */
    ...
    public String openSignaturePad(@ModelAttribute Signature signature,@RequestParam("current_db_pdfname") String current_db_pdfname ,  Model model) {
        ...
        model.addAttribute("current_db_pdfname",current_db_pdfname);
        recordAction(0, signature.getFilename(), actionName.EXIT_PDFPAGE, null, getNowTime(), signature.getUsertype(), null, null);
        log.info("===" + actionName.VISIT_SIGNATUREPAD + "===");
        recordAction(0, signature.getFilename(), actionName.VISIT_SIGNATUREPAD, getNowTime(), null, signature.getUsertype(), null, null);
     ...
     * @param memo
     */
    private void recordAction(int serno, String inputfile, String action, Timestamp begintime, Timestamp endtime, String usertype, String outputfile, String memo) {
        historyMapper.insert(new History(serno, inputfile, action, begintime, endtime, usertype, outputfile, memo));
    }
    ...
    public List save(@RequestParam("fileName") String original_pdfname,@RequestParam("filepath") String original_filepath,
        ...
        //判斷要合併的是哪一個檔案，若fileStatus不存在，就合併原始檔並存入db，反之若存在，且當前使用者身分與存在檔身分一致，就合併存在的那張並更新db，若身分不一致，就再合併到另一張並更新db
        if ("Y".equals(issigned)) {
            ...
            //CurrentPdfnameWhenShowingPdfResult = dbFilename;
            dbSaveType = "UPDATE";
        ...
        //Paths.get("C:","Users","dong","Desktop","signature.png").toFile();
        //將簽名png檔存在temp資料夾中
            ...
            if ("fail".equals(result)) {
                ...
                if ("OK".equals(result)) {
                    if (dbSaveType.equals("UPDATE")) {
                        ...
                        recordAction(0, original_pdfname, actionName.SAVE, getNowTime(), null, Current_Usertype, signature.getFilepath(), null);
